...
const ClientMessages = () => {
  ...
  useEffect(() => {
    const handleAutoSelectChat = async () => {
      if (location.state?.openChatWithContractor && contacts.length >= 0) {
        const contractorId = location.state.openChatWithContractor;
        console.log('Looking for contractor ID:', contractorId, 'in contacts:', contacts);
        const targetContractor = contacts.find(c => c.id === contractorId || c.id === parseInt(contractorId));
        if (targetContractor) {
          console.log('Found target contractor:', targetContractor);
          setSelectedChat(targetContractor.id);
          setCurrentChat(targetContractor);
          // Mark messages as read when auto-selecting
          try {
            if (targetContractor.channel?.id) {
              await messageNotificationService.markChannelAsRead(targetContractor.channel.id);
            }
          } catch (error) {
            console.error('Error marking auto-selected messages as read:', error);
          }
          // Clear the state to avoid reopening on subsequent visits
          navigate('/messages', { replace: true });
        } else {
          console.log('Contractor not found in contacts list, attempting to fetch contractor details');
          // If specific contractor not found, try to fetch contractor details and add to contacts
          fetchContractorAndOpenChat(contractorId);
        }
      }
    };
  ...
  const fetchContractorAndOpenChat = async (contractorId) => {
    try {
      // Ensure contractorId is a number
      const numericContractorId = typeof contractorId === 'string' ? parseInt(contractorId) : contractorId;
      console.log('Creating channel with contractor:', numericContractorId, 'original:', contractorId);
      
      // Create channel between client and contractor
      const channelId = await streamChatService.createChannel(
        null, // clientId will be determined from auth token
        numericContractorId, 
      ...
      // Create a temporary contact entry for this contractor
      const contractorContact = {
        id: numericContractorId,
        name: `Contractor ${numericContractorId}`, // Temporary name, will be updated when messages load
      ...
      setContacts(prev => {
        ...
        const exists = prev.find(c => c.id === numericContractorId || c.id === contractorId);
        if (exists) {
          console.log('Contractor already exists in contacts:', exists);
          return prev;
        }
        console.log('Adding new contractor to contacts:', contractorContact);
        return [...prev, contractorContact];
      });
      
      setSelectedChat(numericContractorId);
      setCurrentChat(contractorContact);
      
      console.log('Set selected chat to contractor:', numericContractorId);
      navigate('/messages', { replace: true });
      
    } catch (error) {
      console.error('Error creating channel with contractor:', error);
      setError('Failed to open chat with contractor');
      // Fallback to first contact if available
  ...
  useEffect(() => {
    ...
    const setupListener = async () => {
      try {
        ...
        // Ensure client exists and channel is watched
        ...
        const handler = (event) => {
          // Only handle messages for the active channel
          if (event?.message && channel?.id === event?.cid?.split(':')[1]) {
            ...
            const mapped = {
              ...
              time: streamChatService.formatTime ? streamChatService.formatTime(new Date(msg.created_at)) : new Date(msg.created_at).toLocaleTimeString(),
            ...
            setMessages(prev => {
              ...
              // Optionally, skip own message if optimistic already added
        ...
        console.error('Error setting up real-time message listener:', e);
      }
    };
  ...
  const loadChannels = async () => {
    try {
      ...
      // Ensure StreamChat is initialized first
      console.log('Initializing StreamChat before loading contacts...');
      await streamChatService.initialize();
      console.log('StreamChat initialized, now loading contacts...');
      
      const hiredContractors = await streamChatService.getContacts();
      setContacts(hiredContractors);
      
      // Only auto-select first contractor if no specific contractor was requested
      if (hiredContractors.length > 0 && !selectedChat && !location.state?.openChatWithContractor) {
        setSelectedChat(hiredContractors[0].id);
        setCurrentChat(hiredContractors[0]);
      }
    } catch (error) {
      console.error('Error loading hired contractors:', error);
      setError('Failed to load hired contractors: ' + error.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const loadMessages = async (contactId) => {
    try {
      // contactId here is the contractor's user ID, not a channel ID
      ...
      if (messages.length > 0 && channelMessages.length > messages.length) {
        ...
        for (const receivedMsg of receivedMessages) {
          await notificationService.notifyMessageReceived(selectedContact.name);
        }
      }
  ...
  const handleSendMessage = async () => {
    ...
    if ((newMessage.trim() || selectedFiles.length > 0) && currentChat) {
      try {
        ...
        // Upload files to backend first if there are any
        ...
        // Notify backend that client actually sent a message (for contractor contact list)
        try {
          const API_BASE_URL = process.env.REACT_APP_API_BASE_URL || 'http://localhost:5000/api';
          const token = authService.getToken();
          
          await fetch(`${API_BASE_URL}/chat/message-sent`, {
            method: 'POST',
            headers: {
              'Authorization': `Bearer ${token}`,
            ...
            body: JSON.stringify({
              contractorId: currentChat.id,
              messageText: newMessage || '[File attachment]'
            })
          });
          
          console.log('Backend notified of message sent to contractor', currentChat.id);
        } catch (notifyError) {
          console.warn('Failed to notify backend of message sent:', notifyError);
        }
        ...
        <div className="client-messages-layout">
          ...
          <div className="client-chat-contacts">
            {loading ? (
              ...
              <div className="no-conversations">No hired contractors found</div>
            ) : (
